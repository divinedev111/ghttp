...
type tlsNextProtoType map[string]func(authority string, c TlsConn) RoundTripper
...
type Transport struct {
	...
	//UConn Func
	ClientFunc func(net.Conn, *tls.Config) (TlsConn, error)

	// Settings is a list of HTTP2 Settings frame options
	HTTP2Settings H2Settings

	WindowUpdateIncrement int

	// HTTP2 Push Handler
	H2PushHandler PushHandler
}
...
func (t *Transport) Clone() *Transport {
	...
	t2 := &Transport{
		...
		Client:                 t.Client,
		ClientFunc:             t.ClientFunc,
	}
	if t.TLSClientConfig != nil {
		t2.TLSClientConfig = t.TLSClientConfig.Clone()
	}
	if !t.tlsNextProtoWasNil {
		npm := make(tlsNextProtoType) //map[string]func(authority string, c TlsConn) RoundTripper
...
func (t *Transport) onceSetNextProtoDefaults() {
	...
	t2.PushHandler = t.H2PushHandler //push promise patch
	if err != nil {
...
// Request.requiresHTTP1
func requiresHTTP1(r *Request) bool {
...
func (t *Transport) connectMethodForRequest(treq *transportRequest) (cm connectMethod, err error) {
	...
	cm.onlyH1 = requiresHTTP1(treq.Request)
	cm.sni = treq.Request.Host
	return cm, err
}
...
func (pconn *persistConn) addTLS(name string, trace *httptrace.ClientTrace) error {
	...
	var tlsConn TlsConn
	if pconn.t.Client != nil {
		tlsConn = pconn.t.Client
	} else if pconn.t.ClientFunc != nil {
		var err error
		tlsConn, err = pconn.t.ClientFunc(plainConn, cfg)
		if err != nil {
			return err
		}
	} else {
		tlsConn = ChromeClientFunc(plainConn, cfg)
	}
	...
	if err := <-errc; err != nil {
		...
		if trace != nil && trace.TLSHandshakeDone != nil {
			trace.TLSHandshakeDone(gotls.ConnectionState{}, err)
		}
		return err
	}
	cs := tlsConn.ConnectionState()
	if trace != nil && trace.TLSHandshakeDone != nil {
		trace.TLSHandshakeDone(convertState(cs), nil)
	}
	pconn.tlsState = &cs
	pconn.conn = tlsConn
	return nil
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		...
		if cm.scheme() == "https" {
			...
			if cm.sni != "" {
				firstTLSHost = cm.sni
			} else if firstTLSHost, _, err = net.SplitHostPort(cm.addr()); firstTLSHost == "" && err != nil {
				return nil, wrapErr(err)
			}
...
type connectMethod struct {
	...
	sni        string // sets as sni if not empty
}
...
func (pc *persistConn) readResponse(rc requestAndChan, trace *httptrace.ClientTrace) (resp *Response, err error) {
	...
	resp.TLS = convertStatePtr(pc.tlsState)
	return
}
...
func (pc *persistConn) writeLoop() {
	...
	for {
		...
		case wr := <-pc.writech:
			...
			err := requestWrite(wr.req.Request, pc.bw, pc.isProxy, pc.waitForContinue(wr.continueCh))
			// err := wr.req.Request.Write(pc.bw)

			if err != nil {
				if bre, ok := get_requestBodyReadError(err); ok {
					err = bre
...
func (pc *persistConn) roundTrip(req *transportRequest) (resp *Response, err error) {
	...
	if pc.t.DisableKeepAlives && !wantsClose(req.Request) {
