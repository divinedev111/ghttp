...
func newTransferWriter(r interface{}) (t *transferWriter, err error) {
	...
	case *Request:
		...
		if te, ok := t.Header["Transfer-Encoding"]; len(t.TransferEncoding) == 0 && ok {
...
func (t *transferWriter) writeHeader2() error {
	if t.Close && !hasToken(t.Header.Get("Connection"), "close") {
	...
	// function of the sanitized field triple (Body, ContentLength,
	// TransferEncoding)
	// fmt.Println("GUYS:", t.ContentLength, t.TransferEncoding, t.shouldSendContentLength(), t.TransferEncoding, t.Trailer)
	if t.shouldSendContentLength() {
	...
	if t.Trailer != nil {
		...
		if len(keys) > 0 {
			...
			// TODO: could do better allocation-wise here, but trailers are rare,
			// so being lazy for now.
			t.Header.Set("Trailer", strings.Join(keys, ","))
		}
	}
