...
func (f *Fingerprinter) FingerprintClientHello(data []byte) (*tls.ClientHelloSpec, error) {
	...
	if ignoredSessionID.Empty() {
		clientHelloSpec.GetSessionID = GetEmptySessionID
	}
	...
	for !extensions.Empty() {
		...
		case extensionServerName:
			...
			for !nameList.Empty() {
				...
				clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.SNIExtension{})

			}
		case extensionNextProtoNeg:
			// draft-agl-tls-nextprotoneg-04
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.NPNExtension{})
		...
		case extensionStatusRequest:
			...
			if statusType == statusTypeOCSP {
				clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.StatusRequestExtension{})
			} else {
		...
		case extensionSupportedCurves:
			...
			for !curvesBytes.Empty() {
				...
				curves = append(curves, tls.CurveID(unGREASEUint16(curve)))
			}
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.SupportedCurvesExtension{Curves: curves})
		...
		case extensionSupportedPoints:
			...
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.SupportedPointsExtension{
				SupportedPoints: supportedPoints,
			})

		case extensionSessionTicket:
			// RFC 5077, Section 3.2
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.SessionTicketExtension{})
		...
		case extensionSignatureAlgorithms:
			...
			for !sigAndAlgs.Empty() {
				...
				supportedSignatureAlgorithms = append(
					supportedSignatureAlgorithms, tls.SignatureScheme(sigAndAlg))
			}
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.SignatureAlgorithmsExtension{SupportedSignatureAlgorithms: supportedSignatureAlgorithms})

		case extensionSignatureAlgorithmsCert:
			// RFC 8446, Section 4.2.3
			if f.AllowBluntMimicry {
				clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.GenericExtension{Id: extension, Data: extData})
			} else {
		...
		case extensionRenegotiationInfo:
			...
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.RenegotiationInfoExtension{Renegotiation: tls.RenegotiateOnceAsClient})
		...
		case extensionALPN:
			...
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.ALPNExtension{AlpnProtocols: alpnProtocols})

		case extensionSCT:
			// RFC 6962, Section 3.3.1
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.SCTExtension{})
		...
		case extensionSupportedVersions:
			...
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.SupportedVersionsExtension{Versions: supportedVersions})
		...
		case extensionKeyShare:
			...
			for !clientShares.Empty() {
				...
				ks.Group = tls.CurveID(unGREASEUint16(group))
			...
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.KeyShareExtension{KeyShares: keyShares})
		...
		case extensionPSKModes:
			...
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.PSKKeyExchangeModesExtension{Modes: pskModes})

		case utlsExtensionExtendedMasterSecret:
			// https://tools.ietf.org/html/rfc7627
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.UtlsExtendedMasterSecretExtension{})

		case utlsExtensionPadding:
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.UtlsPaddingExtension{GetPaddingLen: BoringPaddingStyle})
		...
		case fakeCertCompressionAlgs:
			...
			if !extData.ReadUint8LengthPrefixed(&algoList) || algoList.Empty() {
				return nil, errors.New("unable to read CertificateCompressionAlgorithm extension data")
			}
			fmt.Println(algoList)
			...
			for !algoList.Empty() {
				...
				algorithms = append(algorithms, tls.CertCompressionAlgo(algo))
			}
			fmt.Println(algorithms)
			clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.CompressCertificateExtension{Algorithms: algorithms})
			// clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.GenericExtension{Id: extension, Data: extData})
			// fmt.Println("skipped")
		...
		default:
			if isGREASEUint16(extension) {
				clientHelloSpec.Extensions = append(clientHelloSpec.Extensions, &tls.UtlsGREASEExtension{Value: unGREASEUint16(extension), Body: extData})
