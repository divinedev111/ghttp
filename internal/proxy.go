...
// ProxyDirect is a direct proxy: one that makes network connections directly.
...
func ProxyFromURLnil(u *url.URL) (proxy.Dialer, error) {
	return proxy.FromURL(u, proxy.Direct)
}

func ProxyFromEnvironment() proxy.Dialer {
	return proxy.FromEnvironment()
}

func FromProxy(raw string) (proxy.Dialer, error) {
	proxyURL, err := url.Parse(raw)
	if err != nil {
		return nil, err
	}
	return ProxyFromURL(proxyURL, ProxyDirect)
}
