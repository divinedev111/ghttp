...
func (p *http2clientConnPool) getClientConn(req *Request, addr string, dialOnMiss bool) (*http2ClientConn, error) {
	if http2isConnectionCloseRequest(req) && dialOnMiss {
		...
		cc, err := p.t.dialClientConn(addr, singleUse, req.Host)
		if err != nil {
			return nil, err
		}
		return cc, nil
...
func (fr *http2Framer) checkFrameOrder(f http2Frame) error {
	...
	switch fh.Type {
	case http2FrameHeaders, http2FrameContinuation, http2FramePushPromise:
...
func (fr *http2Framer) readMetaFrame(cont continuable) (http2Frame, error) {
	// if fr.AllowIllegalReads {
	...
	cont.invalidate()
	// mf.http2HeadersFrame.headerFragBuf = nil
	// mf.http2HeadersFrame.invalidate()
	...
	case *http2PushPromiseFrame:
		f = &MetaPushPromiseFrame{http2PushPromiseFrame: orig, Fields: mf.Fields, Truncated: mf.Truncated}
	default:
...
var http2configureTransport = http2configureTransports
...
func (t *http2Transport) dialClientConn(addr string, singleUse bool, host string) (*http2ClientConn, error) {
	var err error
	if host == "" {
...
func (t *http2Transport) newClientConn(c net.Conn, addr string, singleUse bool) (*http2ClientConn, error) {
	...
	cc.fr.ReadMetaHeaders = hpack.NewDecoder(t.get_setting(http2SettingHeaderTableSize, http2initialHeaderTableSize), nil)
	...
	if val, exists := t.t1.HTTP2Settings["HEADER_TABLE_SIZE"]; exists {
	...
	if val, exists := t.t1.HTTP2Settings["ENABLE_PUSH"]; exists {
		initialSettings = append(initialSettings, http2Setting{
	...
	if val, exists := t.t1.HTTP2Settings["MAX_CONCURRENT_STREAMS"]; exists {
		initialSettings = append(initialSettings, http2Setting{
	...
	initialSettings = append(initialSettings, http2Setting{
		...
		Val: t.get_setting(http2SettingInitialWindowSize, http2transportDefaultStreamFlow),
	})

	if val, exists := t.t1.HTTP2Settings["MAX_FRAME_SIZE"]; exists {
		initialSettings = append(initialSettings, http2Setting{
	...
	if val, exists := t.t1.HTTP2Settings["MAX_HEADER_LIST_SIZE"]; exists {
		initialSettings = append(initialSettings, http2Setting{
	...
	windowUpdateIncrement := http2transportDefaultConnFlow
	if t.t1.WindowUpdateIncrement > 0 {
		windowUpdateIncrement = t.t1.WindowUpdateIncrement
	}

	cc.bw.Write(http2clientPreface)
	cc.fr.WriteSettings(initialSettings...)
	cc.fr.WriteWindowUpdate(0, uint32(windowUpdateIncrement))
	cc.inflow.add(int32(windowUpdateIncrement) + http2initialWindowSize)
	cc.bw.Flush()
	if cc.werr != nil {
		cc.Close()
		return nil, cc.werr
...
func (cc *http2ClientConn) roundTrip(req *Request) (res *Response, gotErrAfterReqBodyWrite bool, err error) {
	...
	handleReadLoopResponse := func(re http2resAndError) (*Response, bool, error) {
		...
		res.Request = req
		res.TLS = convertStatePtr(cc.tlsState)
...
func (cc *http2ClientConn) encodeHeaders(req *Request, addGzipHeader bool, trailers string, contentLength int64) (_ []byte, err_ret error) {
	defer func() {
		if e := recover(); e != nil {
			cc.hbuf.Reset()
			cc.henc = hpack.NewEncoder(&cc.hbuf)
			err_ret = fmt.Errorf("HPACEncodeFailed: %v", e)
		}
	}()
	...
	if trailers != "" {
		req.Header.Set("trailer", trailers)
	}

	// Check for any invalid headers and return an error before we
	// potentially pollute our hpack state. (We want to be able to
	// continue to reuse the hpack encoder for future requests)
	keys := getHeaderOrder(req.Header, nil)
	for _, k := range keys {
		if !httpguts.ValidHeaderFieldName(k) {
			return nil, fmt.Errorf("invalid HTTP header name %q", k)
		}
		vv := req.Header.Values(k)
		for _, v := range vv {
			if !httpguts.ValidHeaderFieldValue(v) {
				return nil, fmt.Errorf("invalid HTTP header value %q for header %q", v, k)
			}
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		if req.Method != "CONNECT" {
			pHs[":path"] = path
			pHs[":scheme"] = req.URL.Scheme
		}
		written := map[string]bool{}
		for _, k := range req.Header.Values(pseudoHeaderOrderKey) {
			if v, present := pHs[k]; present && !written[k] {
				f(k, v)
			}
			written[k] = true
		}
		...
		var didUA bool
		// for k, vv := range req.Header {
		for _, k := range getHeaderOrder(req.Header, nil) {
			vv := req.Header.Values(k)
...
func (rl *http2clientConnReadLoop) processHeaders(f *http2MetaHeadersFrame) error {
	...
	if f.StreamEnded() {
		cs.gotEndStream = true //push promise patch
...
func (rl *http2clientConnReadLoop) processData(f *http2DataFrame) error {
	...
	if cs == nil {
		...
		cc.mu.Unlock()
		if f.StreamID >= neverSent && f.StreamID%2 != 0 {
...
func (rl *http2clientConnReadLoop) processPushPromise(f *MetaPushPromiseFrame) error {
	...
	return rl._processPushPromise(f)
}
